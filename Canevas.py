...
import random
from tools import Node, Graph
from tools import ACO
import numpy as np
...
class ZoneAffichage(Canvas):
    ...
    def creer_ligne(self, dx,dy,ax,ay,color):
        ligne = Line(self, dx, dy, ax, ay,color)
        self.pack()
        return ligne
    ...
    def placer_une_ligne_sur_canevas(self, dx,dy,ax,ay,color):
        w,h=self.get_dims()
        #col=['green', 'blue', 'red', 'magenta', 'black', 'maroon', 'purple', 'navy', 'dark cyan']
        #color = np.random.choice(col)
        line = self.creer_ligne(dx,dy,ax,ay,color)
        self.update()
        return line.get_line_ident()
...
class FenPrincipale(Tk):
    def __init__(self):
        ...
        self.__cadre = Frame(self)
        self.__cadre.pack(side =TOP)
        
        self.__cadre2 = Frame(self)
        self.__cadre2.pack(side = BOTTOM)
        
        self.__alpha_lab= Label(self.__cadre,text="alpha").pack(side = LEFT, padx=5,pady=5)
        self.__alpha= Entry(self.__cadre, width = 5)
        self.__alpha.insert(0,'1')
        self.__alpha.pack(side= LEFT,padx=5,pady=5)
        
        self.__beta_lab= Label(self.__cadre,text="beta").pack(side = LEFT, padx=5,pady=5)
        self.__beta= Entry(self.__cadre, width = 5)
        self.__beta.insert(0,'1')
        self.__beta.pack(side= LEFT,padx=5,pady=5)
        
        self.__rho_lab= Label(self.__cadre,text="rho").pack(side = LEFT, padx=5,pady=5)
        self.__rho= Entry(self.__cadre, width = 5)
        self.__rho.insert(0,'0.1')
        self.__rho.pack(side= LEFT,padx=5,pady=5)
        
        self.__nbre_ants_lab= Label(self.__cadre,text="Ants number").pack(side = LEFT, padx=5,pady=5)
        self.__nbre_ants= Entry(self.__cadre, width = 5)
        self.__nbre_ants.insert(0,'20')
        self.__nbre_ants.pack(side= LEFT,padx=5,pady=5)
        
        self.__nbre_iterations_lab= Label(self.__cadre,text="Iteration number").pack(side = LEFT, padx=5,pady=5)
        self.__nbre_iterations= Entry(self.__cadre, width = 5)
        self.__nbre_iterations.insert(0,'100')
        self.__nbre_iterations.pack(side= LEFT,padx=5,pady=5)
        ...
        self.__boutonEffacer = Button(self.__cadre2, text='Effacer le dernier noeud', command=self.undo_last_noeud).pack(side=LEFT, padx=5, pady=5)

        # Création d'un widget Button (bouton Effacer)
        self.__boutonEffacer = Button(self.__cadre2, text='Effacer', command=self.effacer).pack(side=LEFT, padx=5, pady=5)
        
        self.__boutonEffacerligne = Button(self.__cadre2, text = 'Effacer les lignes', command = self.effacer_lines).pack(side = LEFT, padx=5, pady=5)
        # Création d'un widget Button (bouton Quitter)
        self.__boutonQuitter = Button(self.__cadre2, text='Quitter', command=self.destroy).pack(side=LEFT, padx=5, pady=5)
        
        self.boutonCalculer = Button(self.__cadre2, text ='Calculer', command =self.calcul).pack(side =RIGHT, padx=5, pady=5)
    ...
    def placer_une_ligne(self,dx,dy,ax,ay):
        ligne = self.zoneAffichage.placer_une_ligne_sur_canvevas(dx,dy,ax,ay)
    ...
    #--------------------------
    #
    def effacer_lines(self):
        node_list = self.__liste_coordonnes_centre_des_nodes.copy()
        self.effacer()
        for elt in node_list:
            node= self.__zoneAffichage.placer_un_noeud_sur_canevas(elt[0], elt[1])
            self.add_a_node_to_your_list(node)
            self.__liste_coordonnes_centre_des_nodes.append((elt[0], elt[1]))
    ...
    def calcul(self):
        ...
        nbre_iterations = int(self.__nbre_iterations.get())
        nbre_ants = int(self.__nbre_ants.get())
        rho = float(self.__rho.get())
        best_path = aco.solve(alpha= alpha, beta=beta, rho=rho, n_ants=nbre_ants, n_iterations=nbre_iterations, verbose=False)
        
        #Affichage de la distance parcourue
        print ('La distance parcourue est :', best_path[1])
        
        #Lecture de la solution : liste de la suite des noeuds constituant la solution
        node_coord = [(best_path[0][i].x, best_path[0][i].y) for i in range(len(best_path[0]))]
        l=len(node_coord)
        
        #Procéder au tracé des lignes
        L=[0]*l
        color = np.random.choice(self.__color)
        self.__color.remove(color)
        for i in range(l-1):
            L[i]=self.__zoneAffichage.placer_une_ligne_sur_canevas(node_coord[i][0], node_coord[i][1],node_coord[i+1][0], node_coord[i+1][1],color)
        
        #Boucler la trajectoire
        L[l-1]=self.__zoneAffichage.placer_une_ligne_sur_canevas(node_coord[l-1][0], node_coord[l-1][1],node_coord[0][0], node_coord[0][1],color)
        
        #insérer la distance minimale obtenue
        self.__textoutput.insert(INSERT,'La distance parcourue est :'+ str(best_path[1]))
